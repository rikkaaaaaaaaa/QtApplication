#include "deletebus.h"
#include "ui_deletebus.h"

DeleteBus::DeleteBus(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::DeleteBus)
{
    ui->setupUi(this);
...
void DeleteBus::on_buttonBox_accepted()
{
    m_nDeleteBusNumber = m_getBusNumberFromWidget().toInt();

}

void DeleteBus::m_importBusNumberToComboBox(BusInfoTable t_tempBusInfoTable)//向下拉框中引入数据
{
...
void DeleteBus::on_TodayButton_clicked()
{
    ui->comboBox->clear();//清空下拉框中数据

    for(int i = 0;i < m_DeleteBusInfo.m_arrBusToday.size() ; i++ )//循环赋值
    {
        ui->comboBox->addItem(m_TodayTicketBusNumber[i]);//导入车次号数据进入下拉框
    }
    if(m_TodayTicketBusNumber.size() == 0)
    {
        QMessageBox::critical(this, "错误", "还未导入车辆数据，无法移除车辆");
        ui->buttonBox->setEnabled(false);
    }
}


void DeleteBus::on_TomorrowButton_clicked()
{
    ui->comboBox->clear();//清空下拉框中数据

    for(int i = 0;i < m_DeleteBusInfo.m_arrBusTomorrow.size() ; i++ )//循环赋值
    {
...
void DeleteBus::on_DayAfterButton_clicked()
{
    ui->comboBox->clear();//清空下拉框中数据

    for(int i = 0;i < m_DeleteBusInfo.m_arrBusDayAfter.size() ; i++ )//循环赋值
    {
...
QString DeleteBus::m_getBusNumberFromWidget()
{
    QString t_String  = ui->comboBox->currentText();//将ComboBox的内容转化为文本并用一个临时变量承接

    return t_String;//返回值
}


int DeleteBus::DateSelection()//选择天数判断函数
{
    if(ui->TodayButton->isChecked())//今天按钮被勾选
    {
        m_nDeleteBusDate = 0;//日期赋值为0，今天
    }
    else if(ui->TomorrowButton->isChecked())//明天按钮被勾选
    {
        m_nDeleteBusDate = 1;//日期赋值为1，明天
    }
    else if(ui->DayAfterButton->isChecked())//后天按钮被勾选
    {
        m_nDeleteBusDate = 2;//日期赋值为2，后天
    }
    else
    {
        m_nDeleteBusDate = -1;//日期赋值为-1，报错
    }
    return m_nDeleteBusDate;
}
